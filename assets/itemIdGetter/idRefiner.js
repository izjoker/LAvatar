const itemSpecs = require('./itemSpecs.json')
const partsMap = {
    "0": "weapon",
    "1": "avatar-hat",
    "2": "avatar-top",
...
const getStringId = (itemSpec) => {
    let itemId_num = itemSpec['id_num']
    let suffix = ''
...
function classificateByClassName(idLst, className){
    for (let itemId of idLst){
        ...
        let classId = itemId.slice(0, 3)
        let className_ = classIds[classId]
...
const findId = (itemName, className) => {
    let idLst = findAllIdsByName(itemIds, itemName)
    if (idLst.length === 0){
...
for(let numId of Object.keys(itemSpecs)){    
