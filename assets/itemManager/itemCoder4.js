...
const idRefine = require('./idRefiner');
const fs = require('fs');
...
const getRootClass = (className) => {
    for (const rootClass in classes) {
...
const refineIds = (obj) => {
    for (const key in obj) {
        for (const containedItem of obj[key]['contain']) {
...
const rootclassBracketFormatter = (rootClass) => {
    if (rootClass.split('-')[1] !== undefined) {
        const head = rootClass.split('-')[0];

        const gender = rootClass.split('-')[1];
...
const Build_romance = (obj) => {
    const rootClasses_ = _.cloneDeep(rootClasses);
    ...
    for (const design of designs) {
        for (const childId_ of getChildrenId(obj[`${design} 아바타 세트`])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const petName of petNameLst) {
...
const Build_slayer = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`양립하는 분노 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['양립하는 분노 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_2023neria3rd = (obj) => {
    ...
    for (const containedItem of obj['2022 3rd 네리아의 드레스룸']['contain']) {
        for (const itemId of containedItem['itemIdLst']) {
...
const Build_witcher = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`더 위쳐 아바타 상자`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    designs = ['위쳐'];
    containedItems = [];
    const weapons_ = _.cloneDeep(weapons);

    for (const className in weapons_) {
        for (design of designs) {
    ...
    for (const childId of getChildrenId(obj['더 위쳐 무기 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_anniv4thData = (obj) => {
    ...
    // 아바타
    designs = ['용기', '마음'];
    const setName = ['4주년 용기 아바타', '4주년 마음 아바타'];
    for (const key in avatarPartsNameMap) {
        for (const design of designs) {
            for (const rootClass of rootClasses) {
                if (key === '얼굴1') {
                    if (genderFilter(rootClass) === 'm') {
    ...
    // 화신 탈것
    for (const containedItem of obj['봉인된 화신 탈 것 선택 상자']['contain']) {
        for (const itemId of containedItem['itemIdLst']) {
    ...
    for (const containedItems_ of obj['키스 펫 선택 상자']['contain']) {
        for (const itemId of containedItems_[`itemIdLst`]) {
...
const Build_gothicData = (obj) => {
    ...
    // 고양이 펫
    for (const containedItem of obj['밤 고양이 선택 상자']['contain']) {
    ...
    // 왕좌
    for (const containedItem of obj['장미의 왕좌 선택 상자']['contain']) {
        const name = containedItem['itemIdLst'][0];
...
const Build_2ndNeriaData = (obj) => {
    ...
    for (const containedItem of obj['2022 2nd 네리아의 드레스룸']['contain']) {
        for (const itemId of containedItem['itemIdLst']) {
            for (const key of Object.keys(partsMap)) {
...
const Build_2022beachData = (obj) => {
    ...
    for (const partsName in avatarPartsNameMap_) {
        for (design of designs) {
            for (const rootClass of rootClasses) {
    ...
    // 화신 탈것
    for (const containedItem of obj['윈드서핑 세트 선택 상자']['contain']) {
        for (const itemId of containedItem['itemIdLst']) {
            obj[itemId] = new LAItem(itemId, 'mount', [], [], true);
        }
    }
    ...
    for (const containedItems_ of obj['개구리 펫 선택 상자']['contain']) {
        for (const itemId of containedItems_[`itemIdLst`]) {
            obj[itemId] = new LAItem(itemId, 'pet', [], [], true);
        }
    }
...
const Build_DarkPriestData = (obj) => {
    ...
    // 아바타선택 - 전직업
    containedItems = [];
    const designs = ['단죄의 프리스트', '심판의 프리스트'];
    for (const design of designs) {
...
const Build_weatherMancerData = (obj) => {
    ...
    // 아바타선택 - 전직업
    containedItems = [];
    const designs = ['구름의 이변', '안개의 이변'];
    for (const design of designs) {
        const className = '기상술사';
        containedItems.push(new ContainedItem(className, design, 'avatar', [`${design} 세트`]));
    }
    ...
    for (const childId_ of getChildrenId(obj[`기상 이변 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_heavenData = (obj) => {
    ...
    // 무기
    for (const containedItem of obj['천공의 권능 무기 선택 상자']['contain']) {
        const itemId = containedItem['itemIdLst'][0];
    ...
    // 무기
    for (const containedItem of obj['기상술사 천공의 권능 무기 선택 상자']['contain']) {
        const itemId = containedItem['itemIdLst'][0];
        obj[itemId] = new LAItem(itemId.split('_')[0], 'weapon', [itemId.split('_')[1]], [], true);
    }
...
const Build_2022newyearData = (obj) => {
    ...
    for (const design of designs) {
        for (const childId of getChildrenId(obj[`${design} 세트`])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
            obj[childId] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    for (const childId of getChildrenId(obj['새해의 기원 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_painterData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`차원과 환영 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['차원과 환영 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_winterBless = (obj) => {
    ...
    for (const name of mountNameLst) {
...
const Build_2021_2ndNeriaData = (obj) => {
    // 패키지
    const containedItems = [];
    const designs = ['파자마 파티', '겨울 코트 룩'];

    const rootClasses_ = rootClasses;
    ...
    for (const childId of getChildrenId(obj['2021 2nd 네리아의 드레스룸'])) {
        const className = childId.split('_')[1];
        const itemName = childId.split('_')[0];
        let type_ = '';
        for (const partsName in avatarPartsNameMap_) {
...
const Build_3rdAnnivData = (obj) => {
    ...
    for (const rootClass of rootClasses) {
        for (const design of designs) {
            for (const childId of getChildrenId(obj[`${design} 아바타 세트 [${rootClass}]`])) {
                const itemName = childId.split('_')[0];
                const className = childId.split('_')[1];
                const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
                obj[childId] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    for (const childId of getChildrenId(obj['함께한 시간 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
    ...
    const mountNameLst = [
        ...
        '탈 것 : 가이츠 로더 - M',
        '탈 것 : 가이츠 로더 - G'];
    for (const mountName of mountNameLst) {
    ...
    for (const childId_ of getChildrenId(obj[`함께한 시간 아바타 상자 [${rootClass}]`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj[`함께한 시간 무기 상자 [${className}]`])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_2021_1stNeriaData = (obj) => {
    ...
    for (const childId of getChildrenId(obj['2021 1st 네리아의 드레스룸'])) {
        const className = childId.split('_')[1];
        const itemName = childId.split('_')[0];
        let type_ = '';
        for (const partsName in avatarPartsNameMap_) {
...
const Build_glideData = (obj) => {
    ...
    for (const childId of getChildrenId(obj['네온 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_sorceressData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`원소의 축복 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['원소의 축복 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_2021summerData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`한여름밤의 꿈 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
            obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    for (const childId of getChildrenId(obj['한여름밤의 꿈 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
    ...
    for (const childId of getChildrenId(obj['소서리스 한여름밤의 꿈 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
    ...
    for (const childId of getChildrenId(obj['한여름밤의 꿈 악기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_petraniaData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`페트라니아의 기사 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['페트라니아의 기사 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_sosData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`슈테른의 그림자 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['슈테른의 그림자 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_strikerData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`무도의 길 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['무도의 길 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_springDreamData = (obj) => {
    ...
    for (const className in weapons_) {
        const rootClass = getRootClass(className);
        for (design of designs) {
    ...
    for (const childId of getChildrenId(obj['봄날의 꿈 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
    ...
    for (const childId of getChildrenId(obj['스트라이커 봄날 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_gunslingerData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`의뢰의 시작 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['의뢰의 시작 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_2021newyearData = (obj) => {
    ...
    // 아바타선택 - 전직업
    containedItems = [];
    const designs = ['청아한 가람', '우아한 노을', '눈부신 나래', '고고한 달무리', '화려한 꽃보라'];
    for (const design of designs) {
        containedItems.push(new ContainedItem('', design, 'avatar', [
            `${design} 세트_남`,
        ]));
    ...
    for (const design of designs) {
        containedItems = [];
        for (const rootClass of rootClasses_m) {
            containedItems.push(new ContainedItem(rootClass, design, 'avatar', [
    ...
    for (const design of designs) {
        containedItems = [];
        for (const rootClass of rootClasses_f) {
            containedItems.push(new ContainedItem(rootClass, design, 'avatar', [
    ...
    for (const childId of getChildrenId(obj[`도령과 낭자 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    // 아바타
    for (const childId of getChildrenId(obj[`스트라이커 설빔 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_festivalData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`축제와 무도회 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['축제와 무도회 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_anniv2ndData = (obj) => {
    ...
    for (const rootClass of rootClasses_) {
        containedItems = [];
        const rootClass_bracket = rootclassBracketFormatter(rootClass);
        for (const design of designMap[rootClass]) {
            containedItems.push(new ContainedItem(rootClass, design, 'avatar', [
                `${design} 머리_${rootClass}`,
                `${design} 상의_${rootClass}`,
                `${design} 하의_${rootClass}`,
    ...
    for (const childId of getChildrenId(obj[`모험가의 아바타 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId_ of getChildrenId(obj['모험가의 무기 상자'])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_2021halloweenData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`할로윈 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    containedItems = [];
    const weapons_ = _.cloneDeep(weapons);
    const weapons_id_ = _.cloneDeep(weapons_id);
    delete weapons_['기상술사'];
    delete weapons_['도화가'];
    delete weapons_['소서리스'];
    delete weapons_['스트라이커'];
    ...
    for (const childId of getChildrenId(obj['할로윈 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_reaperData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`에이전트 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['에이전트 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_piratesData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`해적왕 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_scouterData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`로보틱스 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['홍염과 청염의 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_swimsuitsData = (obj) => {
    ...
    for (const design1 of designs1) {
        for (const childId of getChildrenId(obj[`${design1} 수영복 선택 상자`])) {
            for (const childId_ of getChildrenId(obj[childId])) {
                const itemName = childId_.split('_')[0];
                const className = childId_.split('_')[1];
                const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['수영복 무기 아바타 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
    ...
    for (const childId of getChildrenId(obj['수영복 악기 아바타 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'instrument', [className], [], true);
...
const Build_omenData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`오멘 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['오멘 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_runnerData = (obj) => {
    ...
    for (const design of designs) {
        for (const childId_ of getChildrenId(obj[`${design} 얼굴 장식 세트`])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
            obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_2020newyearData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`2020 설빔 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_holyknightData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`성직자와 징벌자 아바타 선택 상자`])) {
        for (const childId of getChildrenId(obj[childId_])) {
            const itemName = childId.split('_')[0];
            const className = childId.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['심판자의 한손검 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_2019christData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`밤의 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['밤의 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_anniv1stData = (obj) => {
    ...
    for (const design of designs) {
        for (const childId_ of getChildrenId(obj[`${design} 세트`])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
            obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    for (const childId of getChildrenId(obj['아크라시아의 무기 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_2019halloweenData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`힙스터 아바타 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['힙스터 무기 아바타 선택 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_magicschoolData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`마법학회 교복 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj[`마법학회 생활복 선택 상자`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_faurenzData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj['파우렌츠 아바타 세트 상자'])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_assassinData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`블레이드 아바타 세트`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
    ...
    for (const childId_ of getChildrenId(obj[`데모닉 아바타 세트`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_2019swimsuitsData = (obj) => {
    ...
    for (const design1 of designs1) {
        for (const childId of getChildrenId(obj[`${design1} 아바타 세트`])) {
            for (const childId_ of getChildrenId(obj[childId])) {
                const itemName = childId_.split('_')[0];
                const className = childId_.split('_')[1];
                const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
    ...
    for (const childId of getChildrenId(obj['수영복 무기 아바타 상자'])) {
        const itemName = childId.split('_')[0];
        const className = childId.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true);
...
const Build_umarData = (obj) => {
    ...
    for (const childId of getChildrenId(obj[`우마르의 역작 아바타 세트`])) {
        for (const childId_ of getChildrenId(obj[childId])) {
            const itemName = childId_.split('_')[0];
            const className = childId_.split('_')[1];
            const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
...
const Build_roseData = (obj) => {
    ...
    for (const rootClass of rootClasses_) {
        if (genderFilter(rootClass) === 'm') {
            const design = '검은 장미의 댄서';
            containedItems.push(new ContainedItem(rootClass, design, 'avatar', [
                `${design} 머리_${rootClass}`,
                `${design} 상하의_${rootClass}`,
            ]));
        } else {
            const design = '붉은 장미의 무희';
            containedItems.push(new ContainedItem(rootClass, design, 'avatar', [
                `${design} 머리_${rootClass}`,
                `${design} 상하의_${rootClass}`,
            ]));
    ...
    for (const childId_ of getChildrenId(obj[`장미 아바타 세트`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_2019newyearData = (obj) => {
    ...
    for (const childId_ of getChildrenId(obj[`은가람과 은가비 아바타 세트`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_2018christData = (obj) => {
    ...
    containedItems = [];
    for (const leafClass of classes_lst_) {
        if (genderFilter(leafClass) === 'm') {
            design = '노엘';
        } else {
            design = '이브';
        }
        const rootClass = leaf2RootMap[leafClass];
        const classSigniture = rootClassMap[rootClass];
        const weaponSigniture = weaponSignitureMap[leafClass];
    ...
    for (const childId_ of getChildrenId(obj[`노엘과 이브 아바타 세트 (직업 전용)`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_launchData = (obj) => {
    ...
    for (const leafClass of classes_lst_) {
        const rootClass = leaf2RootMap[leafClass];
        const classSigniture = rootClassMap[rootClass];
        const design = leafSignitureMap[leafClass];
    ...
    for (const childId_ of getChildrenId(obj[`런칭 한정 아바타 세트 (직업 전용)`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_legend1Data = (obj) => {
    ...
    for (const leafClass of classes_lst_) {
        const weaponDesigns = weaponMap[leafClass];
        const designs = designs4Leaf[leafClass];
        for (let i=0; i<2; i++) {
            const design = designs[i];
            const weaponDesign = weaponDesigns[i];
            containedItems.push(new ContainedItem(leafClass, design, 'avatar', [
    ...
    for (const childId_ of getChildrenId(obj[`전설 아바타 시즌1: 약속`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
...
const Build_legend2Data = (obj) => {
    const rootClasses_ = _.cloneDeep(rootClasses);
    rootClasses_.push('전사-여');
    const classes_lst_ = _.cloneDeep(classes_lst);
    ...
    for (const childId_ of getChildrenId(obj[`전설 아바타 시즌2: 도약`])) {
        const itemName = childId_.split('_')[0];
        const className = childId_.split('_')[1];
        const partsName = itemName.split(' ')[itemName.split(' ').length - 1];
        obj[childId_] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true);
