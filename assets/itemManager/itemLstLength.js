...
const Build_anniv4thData = (obj) => {
    ...
    for (key in avatarPartsNameMap){
        for (design of designs){
            for (rootClass of rootClasses){
                if (key === "얼굴1"){
                    if (genderFilter(rootClass) === 'm'){
    ...
    for (let key in weapons) {
        for (design of designs)
            obj[`4주년 ${design} ${weapons_id[key]}`] = new LAItem(`4주년 용기 ${weapons[key]}`, "weapon", [key], [], true)
    }
    // "봉인된 화신 탈 것 선택 상자",
    containedItems = []
...
const Build_2022beachData = (obj) => {
    ...
    //아바타선택 - 전직업
    containedItems = []
    let designs = ["라일라이 비치웨어", "티카티카 비치웨어", "테돈바드 비치웨어", "오레하 비치웨어", "나스카 버블배스"]
    for (let design of designs){
...
const Build_weatherMancerData = (obj) => {
    ...
    obj["기상 이변 아바타 선택 상자"] = new LAItem("기상 이변 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_preppyData = (obj) => {
    ...
    //아바타선택 - 전직업
    containedItems = []
    let designs = ["산뜻한 프레피 룩", "편안한 프레피 룩"]
    for (let design of designs){
        containedItems.push(new ContainedItem("", design, "avatar", [`${design} 세트`]))
    }
...
const Build_painterData = (obj) => {
    ...
    obj["차원과 환영 아바타 선택 상자"] = new LAItem("차원과 환영 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_3rdAnnivData = (obj) => {
    ...
    obj[`함께한 시간 아바타 상자 [${rootClass}]`] = new LAItem(`함께한 시간 아바타 상자 [${rootClass}]`, 'avatarPackage', [rootClass], containedItems, true)

    for (let childId of getChildrenId(obj[`함께한 시간 아바타 상자 [${rootClass}]`])){
        let itemName = childId.split('_')[0]
        let className = childId.split('_')[1]
        let partsName = itemName.split(' ')[itemName.split(' ').length - 1]    
        obj[childId] = new LAItem(itemName, avatarPartsNameMap_[partsName], [className], [], true)
...
const Build_sorceressData = (obj) => {
    ...
    obj["원소의 축복 아바타 선택 상자"] = new LAItem("원소의 축복 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_2021summerData = (obj) => {
    ...
    for (let rootClass of rootClasses_){
        for (design of designs){
            containedItems.push(new ContainedItem(className, `${design}`, 'weapon', [`${design} 악기_${rootClass}`]))
        }
    }
...
const Build_gunslingerData = (obj) => {
    ...
    obj["의뢰의 시작 아바타 선택 상자"] = new LAItem("의뢰의 시작 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_2021newyearData = (obj) => {
    ...
    for (let design of designs){
        ...
        obj[`${design} 세트_${rootClass}`] = new LAItem(`${design} 세트_${rootClass}`, 'avatarPackage', rootClasses_, containedItems, true)
    }
...
const Build_anniv2ndData = (obj) => {
    ...
    let designMap = {
        '전사': ['암흑 지배자', '균열 지배자', '혹한 지배자', '창공 지배자'],
        '마법사': [
            '초록 나비의 여신', 
            '분홍 나비의 여신', 
            '붉은 나비의 여신', 
    ...
    for (let rootClass of rootClasses_){
        containedItems = []
        for (let leafClass of classes[rootClass]){
            for (let design of wDesignMap_root[rootClass]){
...
const Build_scouterData = (obj) => {
    ...
    obj["로보틱스 아바타 선택 상자"] = new LAItem("로보틱스 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_swimsuitsData = (obj) => {
    ...
    for (let rootClass of rootClasses_){
        for (design of designs){
            containedItems.push(new ContainedItem(className, `${design}`, 'weapon', [`${rootClassMap[rootClass]}의 ${design} 악기_${rootClass}`]))
        }
    }
...
const Build_holyknightData = (obj) => {
    ...
    obj["성직자와 징벌자 아바타 선택 상자"] = new LAItem("성직자와 징벌자 아바타 선택 상자", "avatarPackage", [], containedItems , true)
...
const Build_2019halloweenData = (obj) => {
    ...
    containedItems.push(new ContainedItem("", "", "weapon", ["힙스터 무기 선택 상자"]))
    ...
    obj["힙스터 무기 선택 상자"] = new LAItem("힙스터 무기 선택 상자", "weaponPackage", Object.keys(weapons_), containedItems, true)
    // 무기

    for (let childId of getChildrenId(obj['힙스터 무기 선택 상자'])){
        let itemName = childId.split('_')[0]
        let className = childId.split('_')[1]
        let partsName = itemName.split(' ')[itemName.split(' ').length - 1]
        obj[childId] = new LAItem(itemName, 'weapon', [className], [], true)
        ...
        for (let design2 of designs2){
            ...
            for (let rootClass of rootClasses_){
                containedItems.push(new ContainedItem(rootClass, `${design2}의 ${design1}`, 'avatar', [
                    ...
                    `${rootClassMap[rootClass]}의 ${design2} 악기_${rootClass}`,
                ]))
...
const Build_weddingData = (obj) => {
    ...
    for (let design of designs){
        ...
        obj[`${design} 아바타 세트`] = new LAItem(`${design} 아바타 세트`, "avatarPackage", [], containedItems , true)            
    }
...
const Build_roseData = (obj) => {
    ...
    obj[`장미 아바타 세트`] = new LAItem(`장미 아바타 세트`, "avatarPackage", [], containedItems , true)            
...
const Build_2019newyearData = (obj) => {
    ...
    obj[`은가람과 은가비 아바타 세트`] = new LAItem(`은가람과 은가비 아바타 세트`, "avatarPackage", [], containedItems , true)            
...
const Build_2018christData = (obj) => {
    ...
    obj[`노엘과 이브 아바타 세트 (직업 전용)`] = new LAItem(`노엘과 이브 아바타 세트 (직업 전용)`, "avatarPackage", [], containedItems , true)            
...
const Build_launchData = (obj) => {
    ...
    obj[`런칭 한정 아바타 세트 (직업 전용)`] = new LAItem(`런칭 한정 아바타 세트 (직업 전용)`, "avatarPackage", [], containedItems , true)            
