...
export class PackageDict {
    ...
    async debug(){
        ...
        let prices = require('./../../../assets/debug/prices_dummy.json')
        let {mergedItems, rests} = await this.assignmentItems_debug(this.constItems, prices)
    ...
    async mainRoutine() {
        ...
        try{
            let prices = await this.lostarkAPI.getItemPriceData()
